...
class TestCredentialPlugin extends SecretsManagerCredentialsPlugin {
  ...
  /**
   * Provides hardcoded Dummy test value for keys to local env. To be used only for build test execution or in test environment.
  ...
  override function retrieveUsernameAndPassword(key : String) : UsernamePasswordPairBase {
    ...
    try {
      ...
      var secret:String
      if(key.equals("wsi.remote.gw.webservice.cc.configurationprovider")){
        secret = "su:gw"
      }
      else{
